...
function rarely() { # these don't get updated very often, if at all
  cd fossil-scm
  fossil-book
  cd ../sqlite
  sqlite-tests
...
if [ ${#*} -lt 1 ]; then # I want it all
  ...
  for t in ${*}; do
    case "${t}" in "help"|"-h")
      dohelp ;; # exits
      "all") all ;; # Yeah, I know I said it above
      "fossil"|"sqlite"|"tcl")
          "${t}"-all
    ;;
    ...
    "book")
      ...
      cd -
    ;;
 "sqlite-code"|"sqlite-forum"|"sqlite-docsrc"|"sqlite-tests"|"sqlite-althttpd") 
      cd sqlite
      "${t}"
      cd -
    ;;
