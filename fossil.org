...
# leave libfossil out of the main loop, but call them specifically, just like fossil-book, sqlite-wasm and sqlite-testing
...
function dohelp() {
  ...
  echo "$0 sqlite-{code|forum|docsrc|tests|althttpd|wasm}: fetches code, forum, docsrc, althttpd and test scripts"
